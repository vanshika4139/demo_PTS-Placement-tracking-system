...
already sends via the configured SMTP server.
...
blank). Calling these functions logs a 'not_configured' row so the
dashboard counts reflect reality (0 delivered) instead of fabricating
...
CommunicationLog.create(...) calls so logging keeps working.
"""
...
def send_sms(to_number, message, organization_id=None, candidate_id=None):
    """No SMS provider is configured yet (.env SMS_API_URL is blank).
    Logs a not_configured attempt and returns False. Replace this body
    with a real API call once a provider (e.g. Twilio, MSG91) is set up."""
...
def send_whatsapp(to_number, message, organization_id=None, candidate_id=None):
    ...
    blank). Logs a not_configured attempt and returns False. Replace this
    body with a real WhatsApp Business API call once configured."""
